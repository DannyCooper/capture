...
class WP_Capture_Admin_Settings {
	...
	public function register_settings_init() {
		...
		add_settings_section(
			'wp_capture_local_storage_section',
		...
		add_settings_section(
			'wp_capture_notifications_section',
		...
		add_settings_section(
			'wp_capture_subscriber_emails_section',
	...
	private function add_settings_fields() {
		...
		add_settings_field(
			'enable_unsubscribe_links',
			__( 'Enable Unsubscribe Links', 'capture' ),
			array( $this, 'enable_unsubscribe_links_callback' ),
			'wp-capture-settings',
			'wp_capture_privacy_section'
		);
	...
	public function enable_local_storage_callback() {
		...
		echo '<input type="checkbox" id="enable_local_storage" name="wp_capture_options[enable_local_storage]" value="1" ' . checked( 1, $enabled, false ) . ' />';
	...
	public function default_success_message_callback() {
		...
		echo '<input type="text" id="default_success_message" name="wp_capture_options[default_success_message]" value="' . esc_attr( $message ) . '" class="regular-text" />';
	...
	public function privacy_policy_text_callback() {
		...
		echo '<textarea id="privacy_policy_text" name="wp_capture_options[privacy_policy_text]" rows="4" cols="50" class="large-text">' . esc_textarea( $text ) . '</textarea>';
	...
	public function data_retention_days_callback() {
		...
		echo '<input type="number" id="data_retention_days" name="wp_capture_options[data_retention_days]" value="' . esc_attr( $days ) . '" min="0" max="3650" />';
	...
	/**
	 * Enable Unsubscribe Links field callback
	 */
	public function enable_unsubscribe_links_callback() {
		$options = get_option( 'wp_capture_options', array() );
		$enabled = isset( $options['enable_unsubscribe_links'] ) ? $options['enable_unsubscribe_links'] : true;
		
		echo '<input type="checkbox" id="enable_unsubscribe_links" name="wp_capture_options[enable_unsubscribe_links]" value="1" ' . checked( 1, $enabled, false ) . ' />';
		echo '<label for="enable_unsubscribe_links">' . esc_html__( 'Generate unsubscribe links in confirmation emails', 'capture' ) . '</label>';
		echo '<p class="description">' . esc_html__( 'Allows subscribers to unsubscribe via email links (GDPR compliance).', 'capture' ) . '</p>';
	}
	...
	public function notify_admin_new_subscriber_callback() {
		...
		echo '<input type="checkbox" id="notify_admin_new_subscriber" name="wp_capture_options[notify_admin_new_subscriber]" value="1" ' . checked( 1, $enabled, false ) . ' />';
	...
	public function admin_notification_email_callback() {
		...
		echo '<input type="email" id="admin_notification_email" name="wp_capture_options[admin_notification_email]" value="' . esc_attr( $email ) . '" class="regular-text" />';
	...
	public function send_subscriber_confirmation_callback() {
		...
		echo '<input type="checkbox" id="send_subscriber_confirmation" name="wp_capture_options[send_subscriber_confirmation]" value="1" ' . checked( 1, $enabled, false ) . ' />';
	...
	public function subscriber_email_from_name_callback() {
		...
		echo '<input type="text" id="subscriber_email_from_name" name="wp_capture_options[subscriber_email_from_name]" value="' . esc_attr( $from_name ) . '" class="regular-text" />';
	...
	public function subscriber_email_from_email_callback() {
		...
		echo '<input type="email" id="subscriber_email_from_email" name="wp_capture_options[subscriber_email_from_email]" value="' . esc_attr( $from_email ) . '" class="regular-text" />';
	...
	public function subscriber_email_subject_callback() {
		...
		echo '<input type="text" id="subscriber_email_subject" name="wp_capture_options[subscriber_email_subject]" value="' . esc_attr( $subject ) . '" class="regular-text" />';
	...
	public function subscriber_email_template_callback() {
		...
		echo '<textarea id="subscriber_email_template" name="wp_capture_options[subscriber_email_template]" rows="12" cols="70" class="large-text code">' . esc_textarea( $template ) . '</textarea>';
	...
	public function sanitize_options( $input ) {
		...
		// Sanitize enable_unsubscribe_links
		$sanitized['enable_unsubscribe_links'] = isset( $input['enable_unsubscribe_links'] ) ? (bool) $input['enable_unsubscribe_links'] : false;
		...
		// Preserve existing options that aren't handled here (like ems_connections)
		$existing_options = get_option( 'wp_capture_options', array() );
