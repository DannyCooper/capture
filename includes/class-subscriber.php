<?php
/**
 * Subscriber class for managing individual subscriber records.
 *
 * This file contains the Subscriber class which handles all operations
 * related to individual subscriber records including CRUD operations.
 *
 * @since      1.0.0
 * @package    Capture
...
class Subscriber {
	...
	private function populate( $data ) {
		...
		$this->status          = isset( $data['status'] ) ? sanitize_text_field( $data['status'] ) : 'subscribed';
	...
	public function save() {
		...
		if ( $this->id ) {
			...
			return false !== $result ? $this->id : new \WP_Error( 'update_failed', \__( 'Failed to update subscriber.', 'capture' ) );
		} else {
	 ...
	 * @return Subscriber|null Subscriber object or null if not found.
	 */
