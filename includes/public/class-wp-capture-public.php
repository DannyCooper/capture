...
class WP_Capture_Public {
	...
	public function handle_form_submission() {
		...
		if ( empty( $email ) || ! is_email( $email ) || empty( $list_id ) || empty( $ems_connection_id ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid input. Please check your email, selected list, and ensure an EMS connection is configured for this form.', 'capture' ) ) );
			return;
		}
		...
		if ( empty( $ems_connections ) || ! isset( $ems_connections[ $ems_connection_id ] ) ) {
			wp_send_json_error( array( 'message' => __( 'EMS connection not found or not configured for this form.', 'capture' ) ) );
			return;
		}
		...
		if ( empty( $provider_key ) ) {
			wp_send_json_error( array( 'message' => __( 'EMS provider not found in connection details.', 'capture' ) ) );
			return;
		}
		...
		if ( ! $ems_service ) {
			wp_send_json_error( array( 'message' => __( 'EMS service not available.', 'capture' ) ) );
			return;
		}
		...
		try {
			...
			if ( $subscribed ) {
				...
				wp_send_json_success( array( 'message' => __( 'Successfully subscribed!', 'capture' ) ) );
			} else {
				wp_send_json_error( array( 'message' => __( 'Could not subscribe. Please try again.', 'capture' ) ) );
			}
		} catch ( Exception $e ) {
			// Log the exception message for admin
			error_log( 'WP Capture EMS Subscription Error: ' . $e->getMessage() );
			wp_send_json_error( array( 'message' => __( 'An unexpected error occurred. Please try again later.', 'capture' ) ) );
		}
