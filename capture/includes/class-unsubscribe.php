...
class Unsubscribe {
	...
	private function process_unsubscribe( $token ) {
		...
		// Delete ALL subscriber records for this email address (global unsubscribe).
		global $wpdb;
		$table_name = Database::get_subscribers_table_name();
		// First check if any records exist for this email.
		$existing_count = $wpdb->get_var(
			$wpdb->prepare(
				'SELECT COUNT(*) FROM ' . esc_sql( $table_name ) . ' WHERE email = %s',
				$decoded['email']
			)
		);
		...
		// Delete all records for this email address.
		$deleted_count = $wpdb->query(
			$wpdb->prepare(
				'DELETE FROM ' . esc_sql( $table_name ) . ' WHERE email = %s',
				$decoded['email']
			)
		);

		$result = ( $deleted_count !== false );

		if ( $result && $deleted_count > 0 ) {
			return array(
				'success' => true,
				'message' => sprintf(
					/* translators: %d: number of forms unsubscribed from */
					_n(
						'You have been successfully unsubscribed from %d form and your data has been removed from our mailing list.',
						'You have been successfully unsubscribed from all %d forms and your data has been removed from our mailing list.',
						$deleted_count,
						'capture'
					),
					$deleted_count
